...
Your basic shell, called `hfsh` (short for Horned Frog Shell, naturally), is
basically an interactive loop: it repeatedly prints a prompt `hfsh> ` (note
...
At this point, `hfsh` is running, and ready to accept commands. Type away!
...
```
prompt> ./hfsh batch.txt
```

One difference between batch and interactive modes: in interactive mode, a
prompt is printed (`hfsh> `). In batch mode, no prompt should be printed.
...
call `exit(0);` in your hfsh source code, which then will exit the shell.
...
* `path`: The `path` command takes 0 or more arguments, with each argument
  ...
  `hfsh> path /bin /usr/bin`, which would add `/bin` and `/usr/bin` to the
